...
class RabbitHolesTests(APITestCase):
    def setUp(self):
        super().setUp()
        self.url = reverse("rabbithole-list")
    ...
    def test_authentication_required(self):
        ...
        User.objects.create_user(
            username="admin", email="rabbitoverlord@test.com", password="rabbits"
        )
    ...
    def test_can_only_see_my_own_rabbit_holes(self):
        ...
        user1 = User.objects.create_user(
            username="user1", email="user1@test.com", password="rabbits"
        )
        user2 = User.objects.create_user(
            username="user2", email="user2@test.com", password="rabbits"
        )
    ...
    def test_creating_rabbit_holes_sets_user_from_automatically(self):
        ...
        correct_user = User.objects.create_user(
            username="bob", password="bob", email="bob@test.com"
        )
        wrong_user = User.objects.create_user(
            username="mavis", password="mavis", email="mavis@test.com"
        )

        self.client.login(username="bob", password="bob")

        data = {"owner": wrong_user.id, "location": "somewhere", "bunnies": []}
    ...
    def test_superuser_can_delete_any_rabbithole(self):
        ...
        user = User.objects.create_user(
            username="user", email="user@test.com", password="rabbits"
        )
    ...
    def test_cannot_exceed_bunnies_limit(self):
        ...
        rabbit_hole = RabbitHole.objects.create(
            owner=user, location="location", bunnies_limit=3
        )
        for name in ["Flopsy", "Mopsy", "CottonTail", "Spike"]:
            Bunny.objects.create(name=name, home=rabbit_hole)
        url = reverse("bunny-list")
        self.client.login(username="user", password="rabbits")
        data = {"name": "Harry", "home": rabbit_hole.location}
